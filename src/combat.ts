...
export const ROOM_LOCK_MS = 30_000; // ms a combat room stays locked on entry
...
export function moodModifiersFor(mood: string | null): MoodModifier {
  ...
  switch (mood) {
    case "aggressive":
      return {
        ...n,
        enemySpeedMult: AGGRESSIVE_ENEMY_SPEED,
        shootCooldownMult: AGGRESSIVE_SHOOT_COOLDOWN,
      };
    case "happy":
      return {
        ...n,
        spawnCountMult: HAPPY_SPAWN_MULT,
        spawnCapMult: HAPPY_CAP_MULT,
      };
    case "sad":
      return { ...n, playerSpeedMult: SAD_PLAYER_SPEED };
    case "calm":
      return { ...n, sprintDisabled: true };
    case "chill":
      return {
        ...n,
        playerAttackCdMult: CHILL_ATTACK_CD_MULT,
        playerDmgMult: CHILL_PLAYER_DMG,
        sprintSpeedMult: CHILL_SPRINT_SPEED,
        staminaRegenMult: CHILL_STAMINA_REGEN,
        hpRegenMult: CHILL_HP_REGEN,
      };
    case "dark":
      return { ...n, enemyHPMult: DARK_ENEMY_HP };
    case "ethereal":
      return { ...n, playerHitChance: ETHEREAL_HIT_CHANCE };
    case "uplifting":
      return { ...n, playerSpeedMult: UPLIFTING_PLAYER_SPEED };
    case "energetic":
      return {
        ...n,
        enemySpeedMult: ENERGETIC_ENEMY_SPEED,
        playerSpeedMult: ENERGETIC_PLAYER_SPEED,
      };
    case "epic":
      return { ...n, enemyDmgMult: EPIC_ENEMY_DMG };
    case "romantic":
      return {
        ...n,
        spawnCountMult: ROMANTIC_SPAWN_MULT,
        spawnCapMult: ROMANTIC_CAP_MULT,
      };
    case "scary":
      return {
        ...n,
        enemyHPMult: SCARY_ENEMY_HP,
        enemyDmgMult: SCARY_ENEMY_DMG,
      };
    case "sexy":
      return {
        ...n,
        enemySpeedMult: SEXY_ENEMY_SPEED,
        enemyDmgMult: SEXY_ENEMY_DMG,
      };
    default:
      return n;
  }
...
export const difficultyFor = (dungeonMs: number) =>
  1 + dungeonMs / DIFFICULTY_SCALE_MS;
...
export function initialDifficultyFromStats(stats: PlayerStats): number {
  ...
  return (
    INITIAL_DIFFICULTY_DAMPING *
    Math.pow(d_attack * d_hp * d_speed * d_sustain, 0.25)
  );
}
...
export function spawnEnemies(
  ...
  wave = 0,
  difficulty = 1,
  modifier: MoodModifier = MOOD_MODIFIER_NONE,
): Enemy[] {
  const h = hashKey(cellKey);
  const count = Math.max(
    1,
    Math.round((6 + (h % 3)) * modifier.spawnCountMult),
  ); // base 6–8
  ...
  for (let i = 0; i < count; i++) {
    ...
    const inset =
      EDGE_INSET_LO + ((seed >>> 8) % (EDGE_INSET_HI - EDGE_INSET_LO));
    ...
    const hp = Math.round(
      (kind === "charger" ? CHARGER_BASE_HP : SHOOTER_BASE_HP) *
        difficulty *
        modifier.enemyHPMult,
    );
    const dmg = Math.round(
      (kind === "charger" ? CHARGER_BASE_DMG : SHOOTER_BASE_DMG) *
        difficulty *
        modifier.enemyDmgMult,
    );
    const spd = Math.round(CHARGE_SPEED * sqrtD * modifier.enemySpeedMult);
    const baseCooldown =
      kind === "charger"
        ? CHARGE_PAUSE
        : (SHOOTER_COOLDOWN / sqrtD) * modifier.shootCooldownMult;
    const initCooldown =
      kind === "charger" ? 0.5 + (seed % 10) * 0.15 : baseCooldown;
    enemies.push({
      id: `${cellKey}:${i}:${wave}`,
      kind,
      x,
    ...
    if (e.kind === "charger") {
      ...
      if (cd <= 0) {
        ...
        return {
          ...e,
          shootCooldown: 0,
          chargeTimeLeft: CHARGE_DURATION,
          chargeDx: dx / dist,
          chargeDy: dy / dist,
          chargeHit: false,
        };
  ...
  for (const proj of projectiles) {
    if (proj.fromPlayer) {
      for (const e of enemies) {
        ...
        if (
          Math.hypot(proj.x - e.x, proj.y - e.y) <
          PLAYER_PROJ_RADIUS + ENEMY_RADIUS
        ) {
      ...
      if (
        Math.hypot(proj.x - pcx, proj.y - pcy) <
        ENEMY_PROJ_RADIUS + PLAYER_RADIUS
      ) {
